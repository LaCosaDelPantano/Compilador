...
int parse_expression(const char* input, int* pos) {
    ...
    if (token.tipo == TOKEN_NUMBER || token.tipo == TOKEN_IDENTIFIER) {
        if (token.tipo == TOKEN_IDENTIFIER) {
            n_temp = handle_identifier(token);
        } else {
            n_temp = atoi(token.value);
        }

        expression_stream << sacar(token.value);
        token = get_next_token(input, pos);

        if (!is_valid_end_token(token) && check_2) {
            report_error("Se esperaba un ';'.", *pos);
        }
        ...
        while (token.tipo == TOKEN_OPERATOR) {
            ...
            if (token.tipo != TOKEN_NUMBER && token.tipo != TOKEN_IDENTIFIER) {
                report_error("Error de sintaxis en expresión.", *pos);
            }
        ...
        std::string expresion = expression_stream.str();
        n_temp = evaluate_expression(expresion);
        update_auxiliary(token.value);
        return n_temp;
    } else {
        report_error("Se esperaba un número o identificador.", *pos);
    }
